...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--infile", dest="infile", action="store", help="image file(s)"
    )
    parser.add_argument(
        "-o",
        "--outdir",
    ...
    parser.add_argument(
        "-n", "--negative", dest="negative", action="store_true", help="negative"
    )
    parser.add_argument(
        "-M", "--mix", dest="mix", action="store_true", help="mix by loudness"
    )
    parser.add_argument(
        "-a",
        "--amplify",
    ...
    parser.add_argument(
        "--start", dest="start", type=int, action="store", default=0, help="Start frame"
    )
    parser.add_argument(
        "--length",
        dest="length",
    ...
    for n, (block, imgfile) in enumerate(
        zip(audio_chunks(data, blocksize), cycle(files))
    ):
        if n < start:
            continue
        if n > start + length:
