...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--soundfile", action="store", type=str, help="soundfile")
    parser.add_argument(
        "-a", "--amount", action="store", type=float, default=1, help="effect strength"
    )
    parser.add_argument(
        "-A",
        "--amplify",
    ...
    for n, (block, image_1, image_2) in enumerate(
        zip(audio_chunks(data, blocksize), cycle(image_set_1), cycle(image_set_2))
    ):
        ...
        image = scrape(bitmap, bitmap_1, bitmap_2, block_channels, height, width, opt)
